...
void VIOManager::initializeVIO()
{
  ...
  cy = cam->cy();
  // 添加：初始化等距投影参数
  if (cam_model_type == "EquidistantCamera") {
    ...
    vk::MEICamera* mei_cam = dynamic_cast<vk::MEICamera*>(cam);
    if (mei_cam) {
      k1 = mei_cam->k1();
      k2 = mei_cam->k2();
      k3 = mei_cam->p1();
    ...
    for (int grid_row = 1; grid_row <= grid_n_height; grid_row++)
    {
      for (int grid_col = 1; grid_col <= grid_n_width; grid_col++)
      {
        ...
        for (float d_temp = d_min; d_temp <= d_max; d_temp += step)
        {
          ...
          if (cam_model_type == "MEICamera") {
            xyz.normalize(); // 得到单位球上的方向向量
            xyz *= d_temp; // 此时 xyz 可以指向相机背面 (z < 0)
          }
          else {
            xyz *= d_temp / xyz[2];
          }
...
void VIOManager::computeProjectionJacobian(V3D p, MD(2, 3) & J)
{
  if(cam_model_type == "Pinhole") {
    ...
    // // --- MEI 相机 (Unified Projection Model) 优化版雅可比 ---
    // const double x = p[0];
    // const double y = p[1];
    // const double z = p[2];
    ...
    // const double y2 = y * y;
    // const double z2 = z * z;
    // const double d2 = x2 + y2 + z2;
    // const double d = std::sqrt(d2);
    
    // // rho = z + xi * d
    // const double rho = z + xi * d;
    ...
    // // 提取公共偏导项: d(rho)/d(x,y,z)
    // // d_rho_dx = xi * x / d
    // // d_rho_dy = xi * y / d
    // // d_rho_dz = 1 + xi * z / d
    // const double xi_d_inv = xi * d_inv;
    // const double common_xy_factor = xi_d_inv * rho2_inv;
    // const double d_rho_dz = 1.0 + xi * z * d_inv;

    // // 计算 J = d(fx*u, fy*v) / d(x,y,z)
    // // Row 0 (du)
    // J(0, 0) = fx * (rho - x2 * xi_d_inv) * rho2_inv;
    // J(0, 1) = -fx * (x * y * common_xy_factor);
    // J(0, 2) = -fx * x * d_rho_dz * rho2_inv;

    // // Row 1 (dv)
    // J(1, 0) = -fy * (x * y * common_xy_factor);
    // J(1, 1) = fy * (rho - y2 * xi_d_inv) * rho2_inv;
    // J(1, 2) = -fy * y * d_rho_dz * rho2_inv;
    // --- MEI 相机极限优化版 ---
    const double x = p[0], y = p[1], z = p[2];
    const double x2 = x * x, y2 = y * y, z2 = z * z;
    const double d_inv = 1.0 / std::sqrt(x2 + y2 + z2); // 一次开方，一次除法
    
    // 核心变量：rho = z + xi * d
    const double d = 1.0 / d_inv; 
    const double rho = z + xi * d;
    
    if (rho < 1e-8) { J.setZero(); return; }

    const double rho_inv = 1.0 / rho;
    const double rho2_inv = rho_inv * rho_inv;
    
    // 关键优化：提取 common_factor = xi * d_inv * rho2_inv
    // 该因子在 4 个分量中被重复使用
    const double common_factor = xi * d_inv * rho2_inv;
    
    // 预计算 z 方向的公共偏导部分
    // d_rho_dz = 1 + xi * z * d_inv
    const double dz_factor = (1.0 + xi * z * d_inv) * rho2_inv;

    // 直接计算并填充
    const double fx_rho2_inv = fx * rho2_inv;
    const double fy_rho2_inv = fy * rho2_inv;
    const double common_xy = x * y * common_factor;

    // Row 0
    J(0, 0) = fx * (rho_inv - x2 * common_factor); // 利用 1/rho - x^2 * (xi/(d*rho^2))
    J(0, 1) = -fx * common_xy;
    J(0, 2) = -fx * x * dz_factor;

    // Row 1
    J(1, 0) = -fy * common_xy;
    J(1, 1) = fy * (rho_inv - y2 * common_factor);
    J(1, 2) = -fy * y * dz_factor;
  } else {
    printf("Unsupported camera model: %s\n!!", cam_model_type.c_str());
    return;
  }
...
void VIOManager::insertPointIntoVoxelMap(VisualPoint *pt_new)
{
  ...
  {
    // =========================================================
    // 【LRU 控制】：容量控制 (防止格子数量无限膨胀)
    // =========================================================
    if (lru_list.size() >= lru_capacity) 
    {
      VOXEL_LOCATION old_key = lru_list.back();
      auto old_iter = feat_map.find(old_key);
      if (old_iter != feat_map.end()) {
          delete old_iter->second; // 释放内存
          feat_map.erase(old_iter); // 删哈希表
          // lru_list.pop_back(); // 删链表
      } else {
                                        ...
                                        Matrix2d &A_cur_ref)
{
  ...
  if (cam_model_type == "MEICamera") {
    double range_ref = xyz_ref.norm();
    xyz_du_ref *= range_ref / xyz_du_ref.norm();
    xyz_dv_ref *= range_ref / xyz_dv_ref.norm();
  } else {
...
void VIOManager::retrieveFromVisualSparseMap(cv::Mat img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  for (int i = 0; i < pg.size(); i++)
  {
    ...
    if (pt_c[2] > 0|| (cam_model_type == "MEICamera" && pt_c[2] + xi * pt_c.norm() > 1e-6))
    {
      V2D px;
  ...
  for (auto &iter : sub_feat_map)
  {
    ...
    if (corre_voxel != feat_map.end())
    {
      ...
      for (int i = 0; i < voxel_num; i++)
      {
        ...
        // if (dir[2] < 0) continue;
        if (cam_model_type == "MEICamera") {
            if (dir[2] + xi * dir.norm() < 1e-6) continue; 
        } else {
            if (dir[2] < 0) continue; 
        }
        // dir.normalize();
    ...
    if (grid_num[i] == TYPE_MAP)
    {
      ...
      for (int u = -patch_size_half; u <= patch_size_half; u++)
      {
        for (int v = -patch_size_half; v <= patch_size_half; v++)
        {
          ...
          double delta_dist;
          if (cam_model_type == "MEICamera") {
            delta_dist = abs(pt_cam.norm() - depth);
          } 
          else {
            delta_dist = abs(pt_cam[2] - depth);
          }
...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  ...
  printf("\033[1;34m+-------------------------------------------------------------+\033[0m\n");
  printf("\033[1;34m| %-29s | %-27zu |\033[0m\n", "Current Feat Map Size", this->feat_map.size());
  printf("\033[1;34m| %-29s | %-27zu |\033[0m\n", "LRU List Size", lru_list.size());
  printf("\033[1;34m| %-29s | %-27.2lf |\033[0m\n", "Avg Points Per Voxel", avg_points_per_voxel);
  printf("\033[1;34m+-------------------------------------------------------------+\033[0m\n");
