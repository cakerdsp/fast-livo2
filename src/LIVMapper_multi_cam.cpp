...
void LIVMapper::precomputeMappingTable(cv::Mat& map_x, cv::Mat& map_y, const cam_params& cp) {
  ...
  if (cp.model == "MEI") {
  ...
  } else if (cp.model == "KB4" || cp.model == "equidistant") {
    // 1. 提取 KB4 (Kannala-Brandt) 标定参数
    double fu = cp.fx;
    double fv = cp.fy;
    double cu = cp.cx;
    double cv = cp.cy;
    ...
    for (int v = 0; v < H; v++) {
        ...
        for (int u = 0; u < W; u++) {
            // --- 步骤 A: 虚拟针孔平面坐标 ---
            // 将目标图像（虚拟平面）的像素 u,v 转换到归一化坐标
            double mx = (u - v_cx) / v_fx;
            double my = (v - v_cy) / v_fy;
            double mz = 1.0;

            // --- 步骤 B: 计算入射角 theta ---
            // r_pixel_normalized = sqrt(mx^2 + my^2)
            // 在单位球/针孔模型下，theta = atan(r)
            double r_norm = std::sqrt(mx * mx + my * my);
            double theta = std::atan(r_norm);
            ...
            double d_theta = theta + k1 * theta3 + k2 * theta5 + k3 * theta7 + k4 * theta9;

            // --- 步骤 D: 映射到原始鱼眼图像像素坐标 ---
            // 投影关系：u' = fu * (d_theta / r_norm) * mx + cu
            // 注意：当 r_norm 极小时（中心点），d_theta / r_norm 趋近于 1
            double scaling = (r_norm > 1e-8) ? (d_theta / r_norm) : 1.0;

            ptr_x[u] = static_cast<float>(fu * scaling * mx + cu);
...
// 重力对齐只在初始时执行一次，作用是将全局坐标系对齐为 z 轴向下。
...
{    
  ...
  // pv_list 是带协方差的特征点列表，包含：
  // 点的位置：body frame、IMU frame、world frame
  // 协方差信息：body_var、var 等
  ...
  if (imu_prop_enable) 
  {
    ekf_finish_once = true;
    latest_ekf_state = _state;
    // 这个值已经在IMU点云去畸变的时候变成lio_time
    latest_ekf_time = LidarMeasures.last_lio_update_time;
    state_update_flg = true;
  }
  // 按照 T x y z qx qy qz qw 的 TUM/EVO 格式将当前帧的位姿写入磁盘。 通常用于 EVO 评估
  if (pose_output_en) 
  {
  ...
  // 发布里程计信息到pubOdomAftMapped对应的话题中
  euler_cur = RotMtoEuler(_state.rot_end);
  ...
  {
    // 这一步重利用了pv_list_[i]的结构，把feats_down_body按照优化后的位转换到世界坐标系，然后更新体素地图。
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr &msg_in)
{
  ...
  if (!map_x0.empty() && !map_y0.empty()) 
  {
    ...
    double t1 = omp_get_wtime();
    printf("\033[95m remap time: %lf \033[0m", t1 - t0);
  }
  else 
  {
    // 如果表没出来（理论上不会），保底使用原图
...
// 把lid_raw_data_buffer、img_buffer、imu_buffer中的数据打包进meas中，为下一次更新准备数据
// 在LIVO模式下，严格保证LIO VIO交叉进行
// 牢记一点，雷达更新时间就是图像帧到达的时间,相机更新时间也是图像帧到达时间。
// 中间会根据相机图像帧去将点云分开。
// lio_time和vio_time的作用是为了确定下次更新的数据结束位置。
// last_lio_update_time 就是 上次lio_time


// 记住就完了：
// lio_time会被更新为相机时间戳
// 在进行完点云去畸变后，last_lio_update_time = lio_time
// 除此之外， last_lio_update_time只会在未初始化时被初始化为当前雷达帧的起始时间，last_lio_update_time要么是相机时间戳，要么是初始化时的当前雷达帧的起始时间。
// 点云去畸变 -> LIO / VIO
// 在进行LIO过程中，last_lio_update_time 和 lio_time是一样的（因为经过点云去畸变了），都表示这次使用的雷达帧的结束时间（下一次VIO要用图像的时间戳）。
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      *(meas.pcl_proc_cur) = *(meas.pcl_proc_next);
      ...
      // 对后续的雷达帧进行分流，对于正好处于该图像帧之间的点云，其前面一半（以及在之前早于该图像帧但还没拿来更新的点云）被放入pcl_proc_cur，相对时间戳改为针对上一次的lidar更新时间，准备进行下一次更新
      // 后一半被放入pcl_proc_next， 相对时间戳改为对这个图像的位移（lio_times）
      // 这个就要要求VIO和LIO是在同一个时间被更新（也就是每次更新后，last_lio_update_time = lio_time），不然的话，pcl_proc_next移进来的数据是针对上次VIO捕捉的图像的（lio_time），而你现在新加入pcl_proc_cur的点云是相对于上次LIO更新的。
      // 已经确定last_lio_update_time 记录上一次 LIO 更新的时间戳，并在每次 LIO 更新完成后被设置为 lio_time
    ...
    *pcl_wait_pub += *pcl_w_wait_pub;
    if(pub_num >= pub_scan_num)
    {
      ...
      for (size_t i = 0; i < size; i++)
      {
        ...
        // V3D p_body = _state.rot_end.transpose() * (p_w - _state.pos_end);
        // Sphere s(p_body);
        ...
        for (int c = 0; c < camera_num_; c++) {
          ...
          // bool in_gamma = false;
          // if (cu.gamma_wraps) {
          //   // 如果跨越边界，gamma 必须大于 max_gamma 或小于 min_gamma
          //   if (s.gamma >= cu.min_gamma || s.gamma <= cu.max_gamma) in_gamma = true;
          // } else {
          //   // 普通情况，在 [min, max] 之间
          //   if (s.gamma >= cu.min_gamma && s.gamma <= cu.max_gamma) in_gamma = true;
          // }
